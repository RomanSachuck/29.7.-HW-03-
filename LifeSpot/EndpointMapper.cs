...
namespace LifeSpot
{
    public static class EndpointMapper
    {
        ...
        {
            var imgFiles = new[] { "unnamed.jpg", "unnamed1.jpg", "unnamed2.jpg" };
  
            foreach (var fileName in imgFiles)
            {
                builder.MapGet($"/Image/{fileName}", async context =>
                {
                    var imgPath = Path.Combine(Directory.GetCurrentDirectory(), "Image", fileName);
